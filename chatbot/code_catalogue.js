export default {
  async fetch(request, env) {
        ...
        for (const entry of body.entry) {
          for (const webhookEvent of entry.messaging) {
            if (webhookEvent.message && webhookEvent.message.text) {
              ...
              let geminiAnswer = "";
              const pdfUrl =
                "https://pub-cbb59f3933224b2d9a351954294b6b7e.r2.dev/Shalom_C%E1%BB%95%20tr%C3%B2n%20Babytee_Catalogue.pdf";

              // Kiểm tra nếu yêu cầu catalogue hoặc babytee
              if (
                userMessage.includes("catalogue") ||
                userMessage.includes("babytee")
              ) {
                geminiAnswer = `📋 *Catalogue Áo Thun Cổ Tròn Babytee*\n- Sản phẩm: Áo thun 100% cotton 2 chiều\n- Kích thước: Chênh lệch 1-2cm\n- Giá: 140k/áo, giảm còn 120k/áo\n- Quà tặng: Logo yêu thích\n- Freeship: Đặt từ 05 áo\n- Xem chi tiết: ${pdfUrl}\n- Liên hệ: 09.231.294.79 😊`;
              } else {
                // Gọi Gemini trả lời câu hỏi khác
                geminiAnswer = await queryGemini(
                  userMessage,
    ...
    return new Response("Hello from Cloudflare Messenger Bot");
  },
};
...
async function queryGemini(prompt, geminiApiKey) {
  ...
  Thanh toán: 
  "
  Thanh toán cọc 50% trước, số còn lại thanh toán khi nhận hàng.
  "
...
// https://grok.com/chat/b2a5fc4d-4d08-49c2-bf32-3f0cb740fb5f